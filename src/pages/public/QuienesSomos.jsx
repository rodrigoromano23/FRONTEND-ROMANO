...
export default function QuienesSomos() {
  ...
  useEffect(() => {
    async function cargar() {
      console.log("📡 Solicitando sección: quienes-somos");

      try {
        const res = await fetch("http://localhost:4000/api/editor/section/quienes-somos");
        ...
        console.error("❌ Error cargando Quienes Somos:", err);
      }
    }
  ...
  return (
    <div className="p-6">
      <h1 className="text-4xl font-bold mb-6 text-center">Quiénes Somos</h1>

      {publicaciones.map((pub) => (
        <div key={pub._id} className="mb-10 bg-white shadow-lg rounded-xl overflow-hidden">

          {/* ==================== PUBLICACIONES2 (editor) ==================== */}
          ...
          {pub.type === "video" && pub.videoLink && (
            <div className="p-4">
              ...
              <iframe
                width="100%"
                height="315"  // Ajustado a un tamaño más adecuado para pantallas pequeñas
                src={pub.videoLink}
                title="Video"
                frameBorder="0"
          ...
          {(pub.imageUrl || pub.titulo || pub.descripcion) && (
            <div className="p-4">
              ...
              {pub.imageUrl && (
                <img
                  src={pub.imageUrl}
                  className="w-full h-auto mt-2 rounded-md"  // Ajustes responsivos en imágenes
                  alt={pub.titulo || "Imagen de la publicación"}
                />
              )}
            </div>
          )}
